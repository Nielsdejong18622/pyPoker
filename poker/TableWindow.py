...
class TableWithWindow(Table):
    ...
    def __init__(
        ...
        self.canvas = tk.Canvas(self.root, bg="green", width=1024, height=800)
        ...
        # Speed slider.
        tk.Label(self.controls, text="Speed:").pack(side=tk.LEFT)
        self.speed_slider = tk.Scale(
            self.controls, from_=0, to=2000, orient=tk.HORIZONTAL
        )
        self.speed_slider.set(1000)  # type: ignore # default to 1 second
        self.speed_slider.pack(side=tk.LEFT, padx=5)

        # Escape closes window
        self.root.bind("<Escape>", lambda e: self.root.destroy())
        self.root.bind("<space>", lambda e: self.__step_and_draw())
    ...
    def _add_button(self, text: str, command: Callable[[], None]) -> None:
        self._button = tk.Button(self.controls, text=text, command=command)
    ...
    def draw(self, state: "TableState") -> None:
        ...
        self.root.title(f"PokerTable: {state.round}")
        ...
        for i in range(n):
            ...
            self.draw_player(i, px, py, state)
        ...
        for i, card in enumerate(state.cards):
            x = start_x + i * spacing
            y = cy - CARD_HEIGHT // 2
            self.draw_card(x, y, card, face_up=True)

    def draw_card(self, x: float, y: float, card: Card, face_up: bool = True):
        fill = {
            Card.Suit.c: "ORANGE",
            Card.Suit.d: "PINK",
            Card.Suit.h: "GREY",
        ...
        CARD_WIDTH = self.windowsettings.CARD_WIDTH
        CARD_HEIGHT = self.windowsettings.CARD_HEIGHT
        self.canvas.create_rectangle(x, y, x + CARD_WIDTH, y + CARD_HEIGHT, fill=fill)
        self.canvas.create_text(x + CARD_WIDTH / 2, y + CARD_HEIGHT / 2, text=str(card))

    def draw_player(self, index: int, x: float, y: float, state: "TableState"):
        ...
        if at_hand:
            color = "blue"
            self.canvas.create_text(x, y + 50, text=f"${money}", fill="white")
            self.canvas.create_text(x, y + 65, text=f"Bet: {bet}", fill="white")
        elif player.all_in:
            color = "gold"
            self.canvas.create_text(x, y + 50, text=f"${money}", fill="white")
            self.canvas.create_text(x, y + 65, text=f"Bet: {bet}", fill="white")
        ...
        self.canvas.create_text(x, y - 40, text=f"Player {index}", fill="white")

        # Draw buttons.
        if index == state.small_blind_index:
            self.canvas.create_text(x, y, text="SMALL")
        elif index == state.big_blind_index:
            self.canvas.create_text(x, y, text="BIG")

        # Draw hole cards for non-folded players with cards.
        if len(player.cards) == 2 and not folded:
            CARD_WIDTH = self.windowsettings.CARD_WIDTH
            self.draw_card(x + CARD_WIDTH, y, player.cards[0], face_up=True)
            self.draw_card(x + CARD_WIDTH * 2, y, player.cards[1], face_up=True)
    ...
    def __auto_play(self) -> None:
        if self.running:
            ...
            self.root.after(int(delay), self.__auto_play)
